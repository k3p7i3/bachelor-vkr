...
function EditableFeatureField({fieldNames, numericType, feature, setFeature, featureEditIsOverCallcack}) {
  const [isEditActive, setIsEditActive] = useState(false);
  const setIsEditActiveModified = (e) => {
    if (isEditActive && !e) {
      featureEditIsOverCallcack(feature);
    }
    setIsEditActive(e)
  }
  ...
  const changeIsActiveOnEnter = (e) => {
    if (e.key === 'Enter') {
      setIsEditActiveModified(!isEditActive);
    };
  };
  ...
  if (isEditActive) {
    ...
    return <Stack direction="row" sx={{ alignItems: "center"}}>
      ...
      <IconButton onClick={(_) => setIsEditActiveModified(!isEditActive)}>
