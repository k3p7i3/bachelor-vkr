...
import { getActiveValidTariffs } from "./orderCreateSlice"
...
// const mapProduct = (productId, orderProduct) => {
//   var requestProduct = {
//     ...orderProduct,
//     productId: productId,
//     totalNumber: orderProduct.number
//   };
//   delete requestProduct.number;
...
const mapToOrderCalcCostRequest = ({order}) => {
  const reqObj = {};
  const appliedTariffs = order.calcCostForTariffTasks.map((tariffId) => (
    order.appliedTariffs[tariffId]
  ))
...
const mapToOrderCreateRequest = ({order})  => {
