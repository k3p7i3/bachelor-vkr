...
function TypeConditionTableCell({tariff, condition}) {
  const feature = tariff.features.find((f) => {
    return f.id == condition.feature
  })

  const option = feature.options.find((op) => {
    return op.id == condition.option
  })
...
export function ConditionHeaderTableCell({tariff, condition}) {
  ...
  if (condition.type == 'ENUM') {
    const feature = tariff.features.find((f) => {
      return f.id == condition.typeCondition.feature
    })
    conditionHeader = <Typography variant="body1" sx={{ wordBreak: "break-word" }}>{feature.title}</Typography>
  }

  if (condition.type == 'NUMERIC') {
    conditionHeader = <Typography variant="body1" sx={{ wordBreak: "break-word" }}>
